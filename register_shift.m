function [S,warped] = register_shift( image1, image2, thresh, buff );
...
while stop~=1
  ...
  gt=double(im2(buff:fully-buff+1,buff:fullx-buff+1)-image1);
  ...
  if count > 50 | ( abs(local(1)) < thresh & abs(local(2)) < thresh )
        stop=1;  
        warped=im2;
